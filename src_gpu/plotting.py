...
NOTE: This is a CPU-only module that uses Matplotlib. It expects NumPy arrays as input.
"""
import numpy as np # <<< FIX: This must be numpy, not cupy.
...
def plot_results(X, Y, rho, t, ny, problem_type, config_str, save_path=None):
    ...
    Args:
        X, Y (np.array): NumPy meshgrid arrays for coordinates.
        rho (np.array): The final NumPy density field.
    ...
    if problem_type == 'dmr':
        ...
        if np.isnan(min_rho): min_rho = 0.0 # Handle case where all are NaN
        if np.isnan(max_rho): max_rho = 1.0
