# src_gpu/boundary_conditions_gpu.py
"""
...
def apply_boundary_conditions(P_padded, bc_params, t):
    ...
    if problem_type == 'dmr':
        ...
        # --- FIX: Explicitly stack the primitive variables ---
        # Instead of np.array(), use np.stack() for robust CuPy array creation.
        rho_post, u_post, v_post, p_post = cons_to_prim_general(U_post_state[:, np.newaxis])
        P_post_state = np.stack([rho_post, u_post, v_post, p_post])

        rho_pre, u_pre, v_pre, p_pre = cons_to_prim_general(U_pre_state[:, np.newaxis])
        P_pre_state = np.stack([rho_pre, u_pre, v_pre, p_pre])
