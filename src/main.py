...
def run_simulation(args):
    ...
    # Add solver config to params for boundary conditions and RHS
    bc_params = {
        'problem_type': args.problem,
        'reconstruction_scheme': args.scheme,
        'riemann_solver': args.solver,
        'hllc_wave_speed': args.hllc_ws,
        **params # Add all geometric and problem-specific params
    }
    ...
    if args.solver == 'hllc':
        print(f"HLLC Wave Speed: {args.hllc_ws.upper()}")
    ...
    while t < t_final:
        ...
        cs = np.sqrt(GAMMA * p_pres / rho_curr)
    ...
    plot_results(X, Y, rho, t_final, ny, args.problem, config_str)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="A 2D Euler equation solver for CFD benchmarks.")
    
    parser.add_argument('--problem', type=str, default='dmr',
                        choices=['dmr', '2d_riemann', '2d_sod'],
                        help='The benchmark problem to solve.')
                        
    parser.add_argument('--scheme', type=str, default='piecewise_linear',
                        choices=['piecewise_constant', 'piecewise_linear'],
                        help='The spatial reconstruction scheme.')
                        
    parser.add_argument('--integrator', type=str, default='ssprk2',
                        choices=['euler', 'ssprk2'],
                        help='The time integration scheme.')
                        
    parser.add_argument('--solver', type=str, default='hllc',
                        choices=['hll', 'hllc'],
                        help='The Riemann solver to use for numerical flux.')
                        
    parser.add_argument('--hllc_ws', type=str, default='roe',
                        choices=['roe', 'davis'],
                        help='Wave speed estimation method for HLLC solver.')
